...
## E010 — Paired continuation infrastructure

Kaggle 1000-step diagnostic: final six-present-class training mIoU 57.76%, peak logged 77.77% at step 860; not stable near-perfect memorization. Prepared 500-attempt control (4e-4) versus lower-rate (4e-5) continuations from the same saved checkpoint. Adam/scaler restored; legacy RNG unavailable, so both arms restart with seed 3407. Local two-step-per-arm CUDA smoke reproduced source metrics exactly, started both arms identically, saved best/final checkpoint-image pairs and recorded two effective updates each. CPU deliberate-overflow test correctly detected a skipped update. No full LR comparison has been run; no dataset changes or held-out evaluation. See [comparison instructions](../../KAGGLE_LR_COMPARISON.md).
